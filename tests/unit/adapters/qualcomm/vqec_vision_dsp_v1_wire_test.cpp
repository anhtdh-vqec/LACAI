...
using wire = std::array<std::uint8_t, VQEC_VISION_AI_DSP_V1_ENVELOPE_BYTES>;
...
int vqec_vision_ai_unit_dvwrt_test_capability_round_trip() {
    const vqec_vision_ai_dsp_v1_capabilities input{
        VQEC_VISION_AI_DSP_V1_ALL_OPERATIONS, VQEC_VISION_AI_DSP_V1_ENVELOPE_BYTES,
    ...
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_MAJOR_OFFSET] = VQEC_VISION_AI_DSP_V1_MAJOR + 1;
    if (vqec_vision_ai_qcom_dvwir_decode_capabilities(bytes.data(), bytes.size(), &decoded) !=
        vqec_vision_ai_dsp_v1_wire_incompatible) {
        return 1;
    }
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_MINOR_OFFSET] = VQEC_VISION_AI_DSP_V1_MINOR + 1;
    if (vqec_vision_ai_qcom_dvwir_decode_capabilities(bytes.data(), bytes.size(), &decoded) !=
        vqec_vision_ai_dsp_v1_wire_incompatible) {
        return 1;
    }
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_HEADER_BYTES_OFFSET] = 0;
    if (vqec_vision_ai_qcom_dvwir_decode_capabilities(bytes.data(), bytes.size(), &decoded) !=
        vqec_vision_ai_dsp_v1_wire_malformed) {
        return 1;
    }
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_KIND_OFFSET] = 0;
    if (vqec_vision_ai_qcom_dvwir_decode_capabilities(bytes.data(), bytes.size(), &decoded) !=
        vqec_vision_ai_dsp_v1_wire_unsupported) {
        return 1;
    }
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_FIELD_12_OFFSET] = 0;
    if (vqec_vision_ai_qcom_dvwir_decode_capabilities(bytes.data(), bytes.size(), &decoded) !=
        vqec_vision_ai_dsp_v1_wire_malformed) {
        return 1;
    }
...
int vqec_vision_ai_unit_dvwrt_test_request_bounds() {
    const vqec_vision_ai_dsp_v1_capabilities caps{
        1U << (VQEC_VISION_AI_DSP_V1_DENSE_DECODE - 1U), VQEC_VISION_AI_DSP_V1_ENVELOPE_BYTES,
    ...
    const vqec_vision_ai_dsp_v1_request request{
        VQEC_VISION_AI_DSP_V1_DENSE_DECODE, VQEC_VISION_AI_DSP_V1_ENVELOPE_BYTES,
    ...
    if (vqec_vision_ai_qcom_dvwir_validate_request(
            ...
            &decoded) != vqec_vision_ai_dsp_v1_wire_ok ||
    ...
    if (vqec_vision_ai_qcom_dvwir_validate_request(
            ...
            request.output_capacity_bytes, &decoded) != vqec_vision_ai_dsp_v1_wire_malformed ||
        vqec_vision_ai_qcom_dvwir_validate_request(
            &caps, bytes.data(), bytes.size(), request.input_bytes + 1,
            request.output_capacity_bytes, &decoded) != vqec_vision_ai_dsp_v1_wire_out_of_range) {
        return 1;
    }
    const auto original = bytes;
    bytes[VQEC_VISION_AI_DSP_V1_FIELD_24_OFFSET] =
        static_cast<std::uint8_t>(g_request_domain_generation - 1);
    if (vqec_vision_ai_qcom_dvwir_validate_request(
            &caps, bytes.data(), bytes.size(), request.input_bytes, request.output_capacity_bytes,
            &decoded) != vqec_vision_ai_dsp_v1_wire_stale_generation) {
        return 1;
    }
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_KIND_OFFSET] = VQEC_VISION_AI_DSP_V1_IMAGE_TRANSFORM;
    if (vqec_vision_ai_qcom_dvwir_validate_request(
            &caps, bytes.data(), bytes.size(), request.input_bytes, request.output_capacity_bytes,
            &decoded) != vqec_vision_ai_dsp_v1_wire_unsupported) {
        return 1;
    }
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_FIELD_28_OFFSET] = 1;
    if (vqec_vision_ai_qcom_dvwir_validate_request(
            &caps, bytes.data(), bytes.size(), request.input_bytes, request.output_capacity_bytes,
            &decoded) != vqec_vision_ai_dsp_v1_wire_malformed) {
        return 1;
    }
    bytes = original;
    bytes[VQEC_VISION_AI_DSP_V1_FIELD_16_OFFSET] = 0;
    if (vqec_vision_ai_qcom_dvwir_validate_request(
            &caps, bytes.data(), bytes.size(), request.input_bytes, request.output_capacity_bytes,
            &decoded) != vqec_vision_ai_dsp_v1_wire_out_of_range) {
        return 1;
    }
    bytes = original;
    vqec_vision_ai_dsp_v1_capabilities smaller_caps = caps;
