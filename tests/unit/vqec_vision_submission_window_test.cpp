...
int main() {
    ...
    check(window.vqec_vision_ai_core_subwn_reserve(1, 100, 0, first).code_ ==
    ...
    check(window.vqec_vision_ai_core_subwn_reserve(2, 100, 0, first).code_ ==
          status_code::invalid_argument);
    check(window.vqec_vision_ai_core_subwn_reserve(1, UINT64_MAX, 0, first).code_ ==
          status_code::invalid_argument);
    check(window.vqec_vision_ai_core_subwn_reserve(1, 100, 0, first).code_ == status_code::ok);
    check(first.pipeline_pts_ns_ == 1000 && first.source_pts_ns_ == 100);
    submission_ticket second;
    check(window.vqec_vision_ai_core_subwn_reserve(1, 120, 1, second).code_ ==
    ...
    check(window.vqec_vision_ai_core_subwn_reserve(1, 100, 1, second).code_ ==
          status_code::invalid_argument);
    check(window.vqec_vision_ai_core_subwn_reserve(1, 120, 1, second).code_ == status_code::ok);
    ...
    check(draining.vqec_vision_ai_core_subwn_reserve(1, 0, 0, first).code_ == status_code::ok);
    ...
    check(overflow.vqec_vision_ai_core_subwn_reserve(1, 0, 0, first).code_ == status_code::ok);
    check(overflow.vqec_vision_ai_core_subwn_cancel_reserved(first.token_).code_ ==
          status_code::ok);
    check(overflow.vqec_vision_ai_core_subwn_reserve(1, 10, 0, second).code_ ==
          status_code::invalid_argument);
    check(overflow.vqec_vision_ai_core_subwn_reserve(1, 1, UINT64_MAX, second).code_ ==
          status_code::invalid_argument);
