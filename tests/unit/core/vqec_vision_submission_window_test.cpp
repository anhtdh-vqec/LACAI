...
int main() {
    ...
    // The ticket's latency domain must be the steady clock passed to reserve, never the
    // pipeline anchor plus source PTS (a different clock domain).
    check(first.submitted_steady_ns_ == 0 &&
          first.submitted_steady_ns_ != first.pipeline_pts_ns_);
    ...
    check(second.submitted_steady_ns_ == 1);
    check(window.vqec_vision_ai_core_subwn_complete_input(first.token_).code_ ==
          status_code::invalid_state);
