...
Encoder input preparation composes `encoder_window` admission with
`preview_surface_pool` acquisition so a CPU NV12 surface and its ledger ticket stay
together. This document defines the facade, the low-level prepare/cancel helpers and the
backend submission and drain helpers.
...
- Composes encoder admission with preview surface acquisition.
- Keeps the window, ticket and writer together through a bound facade.
- Provides one nonblocking backend drain step.
- Reconciles a committed ticket as input complete plus terminal no-output on a non-ok
  backend return.
- Must not hide loop, sleep, reset or destructor cleanup.
- Must not synthesize completion for a backend that did not complete.
...
does not detect a different nonempty writer. Caller must pass the writer belonging to that
reservation; this helper does not infer token ownership from a raw pointer. Do not call
after submitting the image to hardware. Mutable borrows must end before cancellation or
sealing.
...
- Concrete encoder conformance remains pending.
- The fault-injection cases are synthetic logic evidence, not an executed
  hardware-encoder test or proof of device recovery.
- Populate full NV12, validate authorized overlay, seal, commit before push, retain the
  sealed input and report input/result completion separately; these are not automatic.
