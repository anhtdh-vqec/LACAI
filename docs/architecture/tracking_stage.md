...
**Status:** source-delivered — portable tracking stage source exists.
**Layer:** perception.
**Source:** `src/perception/tracking/vqec_vision_tracking_stage.{hpp,cpp}`.

## Responsibility

- Validate decoder output as detections and the tracker's returned batch as tracked
  observations.
- Own epoch/clock/validation coordination: reset on the first and each strictly newer
  source epoch, and reject a stale epoch rather than resetting backward.
- Must not implement an association algorithm, ByteTrack, optical flow, cross-camera
  identity, persistence or feature decisions.

## Validation and epochs
...
- This source does not implement ByteTrack, optical flow, cross-camera identity,
  persistence or feature decisions.
- A concrete tracker must document its class compatibility, cadence, gap policy, memory
  envelope and golden/replay evidence before activation.
