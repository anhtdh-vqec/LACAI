...
|---|---|---|---|
| Neutral | none | 67/67 | eSDK QEMU; reference backend, runtime executor, service harness, bounded inference worker and tensor pool |
| Expanded | CAMERA, CAMERA_DBUS, GST_FRAME_BRIDGE, QUALCOMM, MODEL_MANIFEST, MODEL_CATALOG, DEPLOYMENT_CONFIG, FEATURE_CATALOG, BUILD_MANIFEST_CHECK, ARTIFACT_DIGEST, QNN_ENGINE | 86/86 | eSDK QEMU; adds camera/D-Bus/GStreamer/Qualcomm/JSON/digest and the owned QNN engine library |
