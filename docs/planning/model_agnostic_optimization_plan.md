...
|---|---|---|---|---|
| S01 | Đã triển khai (device-free) | `qneng_probe_capabilities` chỉ công bố synchronous một job + native output; `supports_async/shared_memory/artifact_update/multi_model_domain = false`; chỉ profile `balanced`; topology `0`. Thêm negative test vào `vqec_vision_inference_execution_test`; bảng capability inventory trong `qualcomm_execution_policy.md`. | eSDK/QEMU expanded 123/123 | Board qualification; async/shared/update/domain chỉ bật khi có lifecycle path + negative tests. O11 vẫn mở ở mức contract. |
| S04 | Một phần (device-free) | O05/O07: engine resolve + cache input/output spec tại `prepare` (validate dtype/shape ở đây, `get_tensors` trả cache, `execute` không dựng lại metadata và so khớp đầy đủ name/shape/dtype/quantization). Pump thêm `resolve_targets`, session gọi sau khi mọi graph running để bỏ lazy lookup trên hot path. | eSDK/QEMU expanded 123/123 | O05 (alloc output bytes mỗi call) và O08 (container churn) cần pool slot + instrumentation; phụ thuộc S05. Chưa có zero steady-state allocation claim. |
| S08 | Một phần (device-free) | O09/O10: multi-input/dynamic/stateful/update/batch bị reject `unsupported` tại activation trước acquire (test multi-input trong `multi_model_pump_tensor_test`); bảng model class coverage trong `model_catalog.md`. O10 đã xử lý ở S04. | eSDK/QEMU expanded 123/123 | Chưa có conformance fixture/board cho từng class; dynamic/stateful/update chỉ bật khi có envelope + pool profile + epoch/reset policy |
| S11 | Một phần (device-free) | Thêm `esdk_configuration_matrix.md` (option/evidence + not-run rule); CI tách job neutral/expanded và gate QAIRT; cập nhật `implementation_status.md` và `qualcomm_adapter.md` để không mô tả plugin là implementation duy nhất; matrix được link từ `esdk_emulation.md`. | eSDK/QEMU expanded 123/123 | CI runner chưa được cấu hình; AST/literal review, sanitizer, neutral include/link dependency check và port-conformance dùng chung reference+adapter còn thiếu. |
| S02 | Một phần (board 2026-09-14) | QNN DSP V68 unit test passed; `qnn-net-run` SCRFD/YOLOv8n smoke; owned engine compose+finalize+execute SCRFD/YOLOv8n với numeric parity byte-identical; latency seed SCRFD avg 5.20 ms, YOLOv8n avg 12.12 ms (sync, chưa gồm preprocess). | Board `/opt/anhtdh`; native 117/117; chi tiết ở `docs/testing/qsc6490_board.md` | Baseline plugin-vs-direct-QNN-vs-CPU đầy đủ, per-KPI theo workload, HTP/CPU/thermal và accuracy nhãn còn thiếu; async/shared/update chưa bật. |
| S03/S05–S07/S09–S10 | Một phần | S03: FastCV preprocess/alignment production path + QNN ION/rpcmem registered buffers delivered | eSDK/QEMU 123/123; `.98` smoke | S05–S07/S09–S10: pooling/async/thermal/accuracy cần board/measurement. |
