...
The original gap was a `service_main` that built a reference graph and synthesized outputs.
The current Qualcomm production owner resolves the package, creates the owned QNN graph,
binds the decoder/tracker and runs real results. Artifact trust and deployment data remain
the authority; `service_main` only forwards validated runtime configuration.
