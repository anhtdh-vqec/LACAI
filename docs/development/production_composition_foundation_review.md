...
| F06 | A pending align completion no longer completes the retained frame ticket; timeout quarantines it and reports recovery-required. | Cascade still executes synchronously on the progress thread; bounded worker, stale completion, reset/quiescence, bounded stop/join and multi-face control-latency proof are missing. |
