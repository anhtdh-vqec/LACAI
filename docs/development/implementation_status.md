...
| `src/adapters/qualcomm/vqec_vision_sdk_loader.cpp` | Private optional QNN backend/system dlopen and interface-provider resolution behind `VQEC_VISION_AI_ENABLE_QNN_ENGINE`; compiles against the vendored QAIRT headers with the eSDK compiler | Backend/device/context creation, capability probe, execute/async/shared-memory/LoRA engine and board qualification |
