...
| `include/vqec/vision/ai/ports/vqec_vision_tracker.hpp` | Neutral serialized tracker port with explicit source-gap and epoch-reset operations | Concrete association/tracking implementation |
...
| `src/app/vqec_vision_perception_result_stage.cpp` | Correlates tensor pipeline PTS with retained source epoch/frame/PTS, then composes model decode and tracking transactionally | Model-slot result routing, concrete decoders/trackers and replay qualification |
| `src/app/vqec_vision_feature_fanout.cpp` | Bounded stable-slot fan-out from one tracked batch with per-feature failure isolation and partial-success reporting | Dependency-aware sharing across model results, effective-state manager and concrete feature rules |
