...
| `src/perception/embedding/vqec_vision_exact_embedding_index.cpp`, `src/adapters/zvec/vqec_vision_zvec_embedding_index.cpp` | Neutral revision-pinned embedding index port, bounded exact cosine reference backend and optional Zvec C API adapter source with model-version isolation, cosine-distance conversion and faulted-mutation gating | Durable encrypted gallery adapter, Zvec package/target qualification, enrollment and calibrated FR policy |
