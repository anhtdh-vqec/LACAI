...
| `src/core/vqec_vision_usecase_activation.cpp` | Transactional pre-load usecase gate composition; filters the maximum deployment to the union of ready root models, preserves shared roots and produces a valid idle plan with no sources | Authenticated usecase catalog/entitlement loader, D-Bus adapter and live runtime-generation reconciliation |
