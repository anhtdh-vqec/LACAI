...
| `src/app/vqec_vision_perception_result_stage.cpp` | Correlates tensor pipeline PTS with retained source epoch/frame/PTS, then composes model decode and tracking transactionally | Model-slot fan-out to feature stages, concrete decoders/trackers and replay qualification |
