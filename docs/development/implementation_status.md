...
| `src/perception/embedding/vqec_vision_recognition_session.cpp`, `src/perception/embedding/vqec_vision_face_enrollment_controller.cpp`, `src/adapters/fw_control/vqec_vision_face_enrollment_dbus.cpp` | Bounded multi-template recognition owner, revision-CAS enrollment/remove lifecycle, exact-frame label correlation and optional DBus control adapter | Durable encrypted gallery/recovery, peer authorization integration, temporal/liveness policy and attendance output |
