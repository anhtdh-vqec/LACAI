...
| `src/adapters/qualcomm/vqec_vision_qnn_engine.cpp` | Private optional LACAI-owned QNN engine: dlopen backend/system, backend/device creation, capability probe, context + single-graph model-lib compose, typed tensor metadata and synchronous client-buffer execute; compiles against vendored QAIRT with the eSDK compiler | Async/shared-memory/LoRA execution, `inference_graph_port` binding, runtime composition wiring and board qualification |
