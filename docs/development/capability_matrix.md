...
| S04 App Manager lifecycle | Ed25519 package/entitlement verification, persistent SQLite inventory, immutable content store, bounded async package journal, update/rollback, configuration/desired CAS, D-Bus facade and reconnecting runtime snapshot | Fresh no-app → asynchronous install → enable, ten toggle cycles, idempotent update/rollback, uninstall/reinstall and 167/167 eSDK tests on `.102` | Remaining mutation migration, backend conformance, deeper fault injection and trust rotation |
