...
| S04 App Manager lifecycle | Ed25519 package/entitlement verification, persistent SQLite inventory, immutable content store, bounded async package journal, update/rollback, configuration/desired CAS, D-Bus facade and reconnecting runtime snapshot | Fresh no-app → install → entitlement → enable, ten toggle cycles and synchronous update/rollback on `.102`; async package journal 167/167 eSDK logic-tested | Async board path, remaining mutation migration, backend conformance, fault injection and trust rotation |
