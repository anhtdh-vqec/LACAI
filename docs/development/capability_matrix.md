...
| Per-app activation delta | Complete-snapshot diff, exact shared-dependency counts, transactional output/feature rebind, restartable primary/secondary slots and global async-worker quiesce barrier | 172/172 eSDK/QEMU; race tests 30x each; native 10x each; `.102` S04 + probe completed 20 five-second transitions, config delta and App Manager restart at one PID/epoch, 30.124 FPS and 13.40% CPU | Second product package quality/resource evidence, released-FW boundary and 18-usecase hardware capacity |
