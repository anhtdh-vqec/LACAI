...
| S04 App Manager lifecycle | Ed25519 package/entitlement verification, bounded S01–S18 catalog/status, persistent SQLite inventory, immutable content store, bounded async package journal, update/rollback, configuration/desired CAS, D-Bus facade and reconnecting runtime snapshot | Fresh no-app → asynchronous install → enable, ten toggle cycles, idempotent update/rollback and uninstall/reinstall on `.102`; 168/168 eSDK tests including eight install/update `SIGKILL` checkpoints, shared-component fixtures and deterministic S04 replay | Electrical power-cut rig, remaining mutation migration, backend conformance and trust rotation |
...
| Performance target | Exact `.102` candidate with registered DMA-BUF and generic cDSP path: 30.124 FPS preview; service 10.50% plus App Manager 3.83% of one logical core over five minutes | Ten toggles pass; FD 73/11 stable; cold-start peak measured at 86% in QNN `GraphPrepare` | 18-usecase capacity, QNN context-binary cold-start work, real S04 overlay/model quality and released-FW thermal/long-soak qualification |
