...
| S04 App Manager lifecycle | Ed25519 package/entitlement verification, bounded S01–S18 catalog/status, persistent SQLite inventory, immutable content store, bounded async journal for every mutation, D-Bus facade and reconnecting runtime snapshot | Complete async v1 wire passes 170/170 eSDK/QEMU; exact `.102` candidate proved fresh no-app → entitlement → install → enable, 18-entry listing, all mutation kinds, restart and ten toggle cycles | Electrical power-cut rig, backend implementation conformance and trust rotation |
| Per-app activation delta | Complete-snapshot diff, exact shared-dependency counts, transactional output/feature rebind, restartable primary/secondary slots and global async-worker quiesce barrier | 172/172 eSDK/QEMU; race tests 30x each; native 10x each; `.102` real-camera S04 + probe completed 20 five-second transitions, config delta and App Manager restart at one PID/epoch, 30.000 FPS and 13.40% CPU | Automatic FastRPC recovery after mid-inference camera loss, second product package quality/resource evidence, released-FW boundary and 18-usecase hardware capacity |
...
| Performance target | Exact `.102` S04-only candidate: 29.795 ring FPS over 300.890 seconds and 30.124 RTSP FPS; service 10.835% plus App Manager 3.759% of one logical core | Ten toggles pass; service FD 74→74; cold-start peak remains separately measured at 86% in QNN `GraphPrepare` | 18-usecase capacity, QNN context-binary cold-start work, real S04 overlay/model quality and released-FW thermal/long-soak qualification |
| Recovery/metrics helpers | Source-loss generation replacement with validated backoff; fatal hardware faults remain fail-closed | More than 120 seconds without compatibility camera, ten-second App Manager restart and reverse startup order pass with the same service PID | BSP reset, released-FW recovery, extended release soak and complete stage metrics |
