...
float vqec_vision_ai_track_ioutr_iou(const overlay_box& _left, const overlay_box& _right) {
...
iou_tracker::iou_tracker(iou_tracker_config _config) noexcept : config_(_config) {}

status iou_tracker::vqec_vision_ai_track_ioutr_validate() const {
    if (config_.iou_threshold_ < iou_tracker_limits::g_min_iou_threshold ||
        config_.iou_threshold_ > iou_tracker_limits::g_max_iou_threshold) {
        return {status_code::invalid_argument, "tracker IoU threshold is out of range"};
    }
    if (config_.max_lost_frames_ == 0 || config_.max_tracks_ == 0 ||
        config_.max_tracks_ > iou_tracker_limits::g_max_tracks) {
        return {status_code::invalid_argument, "tracker track/lost bounds are invalid"};
    }
    return {};
}

status iou_tracker::vqec_vision_ai_ports_trker_validate_activation() const {
    return vqec_vision_ai_track_ioutr_validate();
}

void iou_tracker::vqec_vision_ai_track_ioutr_compact() {
    std::size_t write = 0;
...
status iou_tracker::vqec_vision_ai_ports_trker_update_tracks(
    ...
    const auto valid = vqec_vision_ai_track_ioutr_validate();
    ...
    std::array<bool, iou_tracker_limits::g_max_tracks> matched{};
    ...
    for (auto& item : candidate.observations_) {
        ...
        std::size_t best_track = iou_tracker_limits::g_max_tracks;
        for (std::size_t track = 0; track < track_count_; ++track) {
            if (matched[track]) {
                continue;
            }
            const float iou = vqec_vision_ai_track_ioutr_iou(item.box_, tracks_[track].box_);
            if (iou > best_iou) {
                best_iou = iou;
                best_track = track;
            }
        }
        if (best_track < iou_tracker_limits::g_max_tracks) {
    ...
    vqec_vision_ai_track_ioutr_compact();
    _tracked = std::move(candidate);
    return {};
}

status iou_tracker::vqec_vision_ai_ports_trker_reset_epoch(std::uint64_t _source_epoch) {
    if (_source_epoch == 0) {
