...
public:
    ...
    // Serialized cold/control-path replacement. Count zero clears the binding. Validation is
    // transactional and preserves the previous stages on failure.
    [[nodiscard]] status vqec_vision_ai_appl_ftfan_replace_stages(
        const std::array<feature_stage*, feature_fanout_limits::g_max_feature_stages>& _stages,
        std::uint16_t _stage_count);
