...
//
// Compatibility-named private renderer. The v1 implementation composes NV12 and boxes through
// the generic cDSP operation, imports the AI-owned rpcmem surface into v4l2h264enc and writes
// each H.264 access unit into the released FW ring. No Qualcomm overlay plugin is used.
...
struct qtiv_renderer_config {
    ...
    // Neutral configured colour is 0xRRGGBBAA; alpha is the least-significant byte.
...
public:
    ...
    // Composes a prepared, authorized overlay through cDSP, encodes the frame and writes one
    // AU to the ring. Stale or mismatched observations are dropped.
