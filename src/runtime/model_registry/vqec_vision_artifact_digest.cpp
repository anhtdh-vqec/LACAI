...
status vqec_vision_ai_mreg_ardgt_verify_fd(int _fd, const std::string& _expected_sha256,
                                           std::uint64_t _max_bytes,
                                           artifact_digest_receipt& _receipt) {
    if (_fd < 0) {
        return {status_code::invalid_argument, "artifact descriptor is invalid"};
    }
    if (_expected_sha256.size() != 64 ||
        _expected_sha256.find_first_not_of("0123456789abcdef") != std::string::npos ||
        _max_bytes == 0 || _max_bytes > 4ULL * 1024 * 1024 * 1024) {
    ...
    for (;;) {
        const auto requested = static_cast<std::size_t>(
            std::min<std::uint64_t>(block.size(), _max_bytes - total + 1));
        const auto count = ::read(_fd, block.data(), requested);
        if (count < 0) {
            if (errno == EINTR) {
                continue;
            }
            return {status_code::io_error, "artifact descriptor read failed"};
        }
        if (count == 0) {
            break;
        }
        ...
        if (EVP_DigestUpdate(context.get(), block.data(), static_cast<std::size_t>(bytes)) != 1) {
            return {status_code::io_error, "SHA-256 update failed"};
        }
        total += bytes;
    }
