...
status vqec_vision_ai_core_obval_validate_batch(
    ...
    for (const auto& item : _batch.observations_) {
        ...
        if (item.track_id_ == 0 || !vqec_vision_ai_core_obval_valid_identifier(item.class_id_) ||
            !std::isfinite(item.confidence_) || item.confidence_ < observation_limits::g_min_confidence ||
            item.confidence_ > observation_limits::g_max_confidence || item.attributes_.size() > observation_limits::g_max_attributes) {
        ...
        for (const auto& attribute : item.attributes_) {
            if (!vqec_vision_ai_core_obval_valid_identifier(attribute.schema_id_) ||
                ...
                !std::isfinite(attribute.confidence_) || attribute.confidence_ < observation_limits::g_min_confidence ||
